...
PM1::PM1(unsigned short serial):AquaBusDev(APEX_MODULE_PM1, serial, 0x01, 0x07)
{
  ModuleStatus.ProbeRangeOriginal = eeprom_read_byte((const uint8_t *)3);
	...
	ModuleStatus.pH_DOProbeOffset = eeprom_read_word((const uint16_t *)4);
	ModuleStatus.TempProbeOffset = eeprom_read_word((const uint16_t *) 6);
	ModuleStatus.ORPProbeOffset = eeprom_read_word((const uint16_t *)8);
	ModuleStatus.pH_DOProbeScale = eeprom_read_word((const uint16_t *)10);
	ModuleStatus.TempProbeScale = eeprom_read_word((const uint16_t *)12);
	ModuleStatus.ORPProbeScale = eeprom_read_word((const uint16_t *)14);
	ModuleStatus.Unknown_3 = eeprom_read_word((const uint16_t *)16);
...
void PM1::processData(byte deviceABAddr, byte* data, unsigned short length)
{
  ...
  switch (requestType)
  {
  	...
  	case 2: //calibrate
		  ...
		  eeprom_update_byte(( uint8_t *) 3, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->ProbeRangeOriginal);
		  eeprom_update_word(( uint16_t *) 4, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->pH_DOProbeOffset);
		  eeprom_update_word(( uint16_t *) 6, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->TempProbeOffset);
		  eeprom_update_word(( uint16_t *) 8, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->ORPProbeOffset);
		  eeprom_update_word(( uint16_t *) 10, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->pH_DOProbeScale);
		  eeprom_update_word(( uint16_t *) 12, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->TempProbeScale);
		  eeprom_update_word(( uint16_t *) 14, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->ORPProbeScale);
		  eeprom_update_word(( uint16_t *) 16, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->Unknown_3);
		  eeprom_update_word(( uint16_t *) 18, ((AB_PM1_CALIBRATE_REQUEST_PACKET*)data)->Unknown_4);
